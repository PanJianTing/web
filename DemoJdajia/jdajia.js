...
/**
 * 選擇照片、拍攝照片。相關的方法與設定。
 * type: 數值。 1/2/3 1.相機 2.相簿  3.遮罩相機(方形)
 * current: 數值。目前選擇的數量(預設0)
 * max: 數值。相簿多選的最大數量(5張以內)
 * isReturnOrgUrls: 數值。 0/1 照片路徑是否為原始圖。0:否，1:是，預設為0
 * thumbnailWidth: 照片壓縮寬度(預設300)
 * editImageType: 修改圖片的種類
 * addWaterMark: 數值或字串。 0/1 是否要把圖片加上浮水印(0:不加、1:加上。若為其他值(null)，則視為0的狀況。)
 * waterMarkSetting: 物件類型，此參數需為addWaterMark為1的狀況才有用。
 *
 * waterMarkSetting需傳的參數如下：
 *      x: 數值，0~1之間，x軸的起始點，算法為，x * 原圖寬，若1，則是從原圖寬開始畫。 預設:0
 *      y: 數值，0~1之間，y軸的起始點，算法為，y * 原圖高，若1，則是從原圖高開始畫。 預設:0
 *      width: 數值，0~1之間，浮水印的寬度，算法為，width * 原圖寬，若1，則寬度為原圖寬。 預設:1
 *      height: 數值，0~1之間，浮水印的寬度，算法為，height * t原圖高，若1，則寬度為原圖高。 預設1
 *      str: 字串，浮水印的字串，若沒帶或為空，則為空字串。
 *      fontSize: 數值，12~64之間。浮水印字的大小，預設為16。若字太大，浮水印範圍太小，則會被切掉。 預設12
 *      fontColor: 字串，HEX Code，EX:#FFFFFF 預設:白色
 */
da.chooseImage = function (v) {
    if (this.canTrans())
        djInternal.callHandler('chooseImage', {
                ...
                editImageType: v.editImageType,
                addWaterMark: v.addWaterMark,
                waterMarkSetting: v.waterMarkSetting,
            },
            function (callback) {
                switch (callback.code) {
                    case 'success':
 ...
 * readTypes 取得指定通知狀態的通知數量（0:未讀，1:已讀，2:未讀不顯示），陣列類型，例如：["0","1"]
 * @param v
 */
da.getNotificationCountBySourceType = function (v) {
    if (this.canTrans())
        djInternal.callHandler('getNotificationCountBySourceType', {'sourceType': v.sourceType,
                'subSourceType':v.subSourceType,'readTypes':v.readTypes},
            function (callback) {
                switch (callback.code) {
                    case 'success':
                        v.success(callback.res);
...
/**
 *  新增NavigationBar 右側客製按鈕
 *
 */
da.addCustomRightButton = function (v) {

    djInternal.callHandler('addCustomRightButton', {
				'buttons': v.buttons,	
	},
    function (callback) {
        switch (callback.code) {
...
/**
 *  連續掃描QRCode接口(需與endScanQRCode搭配使用)
 *  title:頂部顯示的標題
 *  callbackFunc:掃描到QRCode要回傳H5頁面的Function
 */
da.startScanQRCode = function (v) {

    djInternal.callHandler('startScanQRCode', {
        'title':v.title,
        'callbackFunc': v.calbackFunc,   
    },
    function (callback) {
        switch (callback.code) {
            case 'success':
 ...
 *  title:頂部顯示的標題
 *  callbackFunc:掃描到QRCode要回傳H5頁面的Function
 *  once:1與0，如果給1則會只掃描一次回傳，給0則為連續掃描。需與endScan一起使用。
 *  shake:0/1 掃描到產生震動
 *  sound:0/1 掃描到產生聲音
 */
da.showScan = function (v) {
    djInternal.callHandler('showScan', {
            'title':v.title,
...
/**
 *  連續掃描QRCode接口(需與startScanQRCode搭配使用)
 *  isEnd:是否結束掃描，0為結束，1為繼續
 *  showAlertStr:提示顯示文字
 *  PS:如果要打Ajax或做非同步的處理，可能會卡住。只能做同步的處理。
 */
da.endScan = function (v) {
    djInternal.callHandler('endScan', {
            'isEnd': v.isEnd,
            'showAlertStr': v.showAlertStr,
        },
        function (callback) {
            switch (callback.code) {
                case 'success':
 ...
 */
da.video = function (v) {
    djInternal.callHandler('video', {
            'type': v.type,
            'resolution': v.resolution,
            'uploadUrl': v.uploadUrl,
            'progressCallBack': v.progressCallBack,
        },
        function (callback) {
            switch (callback.code) {
                case 'success':
 ...
 * 取得使用者配置的首頁自定義應用資訊
 */
da.getCustomizationTopic = function (v) {
    djInternal.callHandler('getCustomizationTopic', {},
        function (callback) {
            switch (callback.code) {
                case 'success':
                    v.success(callback.res);
 ...
 * 設定使用者配置的首頁自定義應用資訊
 * topicList: 使用者所選擇的應用
 * selectTopicPreset: 使用者所選擇App一進入首頁時開啟的應用。
 * customizationListStyle: 使用者選擇首頁自定義該橫列呈現還是直列呈現。 0:橫列 1:直列
 */
da.setCustomizationTopic = function (v) {
    djInternal.callHandler('setCustomizationTopic', {
            'topicList': v.topicList,
            'selectTopicPreset': v.selectTopicPreset,
            'customizationListStyle': v.customizationListStyle,
        },
        function (callback) {
            switch (callback.code) {
                case 'success':
...
/**
 * 取得端上所接收到的Receive/list的CompanyMenu
 * type:0/1/2, 0為根據companyMenuIDs所回傳，1為回傳動態應用授權的應用資訊，2為回傳首頁自定義的應用資訊。
 * companyMenuIDs:要回傳的companyMenu的CompanyMenuID，若Type為1/2，則此參數可不傳。String，已","區隔。格式為12345678,12345678
 * 若為type=2的情況，會多回傳listStyle與selectTopic這兩個值
 * listStyle: 使用者選擇首頁自定義該橫列呈現還是直列呈現。 0:橫列 1:直列
 * selectTopic: 使用者所選擇App一進入首頁時開啟的應用。
 */
da.getReceiveListTopicInfo = function (v) {
    djInternal.callHandler('getReceiveListTopicInfo', {
            'type':v.type,
            'companyMenuIDs':v.companyMenuIDs,
        },
        function (callback) {
            switch (callback.code) {
                case 'success':
...
/**
 * 由手機打向產品或移動後端的接口。
 * url:url(含Http，若沒https or http 則會自己補上移動網址http(s):ip:port/url)
 * type:GET/POST(注意：需大寫)
 * data:傳遞參數
 */
da.sendHttpConnect = function (v) {
    djInternal.callHandler('sendHttpConnect', {
            'url': v.url,
            'type': v.type,
            'data': v.data,
        },
...
/**
 * 20190911 PanJianTing 帳號轉換
 * account:轉換的帳號
 * password:轉換的密碼
 */
da.accountConvert = function (v) {
    djInternal.callHandler('accountConvert', {
            'account': v.account,
            'password': v.password,
        },
        function (callback) {
